...
export function FollowListItem({ entry }: FollowListItemProps) {
  ...
  const content = (
    <div className="flex items-start gap-4 px-1 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="font-mono text-xs text-ash">@{entry.username}</p>
        {entry.bio && <p className="mt-1 line-clamp-2 font-sans text-sm text-ash/80">{entry.bio}</p>}
        {/* Only for a claimed profile (see FollowListEntry's comment) —
            same rule Phase 4 already applies on the profile page itself. */}
        {entry.profileSlug && (
          <div className="mt-2">
            <DiscordBadge serverMember={entry.serverMember} />
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <Link
      href={buildProfileURL(entry.profileSlug)}
