...
export function FollowListModal({ isOpen, onClose, title, endpoint, emptyMessage }: FollowListModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      {error && (
        <p className="py-4 text-center font-sans text-xs text-ash">Не вдалося завантажити список.</p>
      )}

      {!error && entries === null && (
        <p className="py-4 text-center font-sans text-xs text-ash">Завантаження…</p>
      )}
      ...
      {!error && entries !== null && (
        <>
          ...
          {entries.length === 0 && (
            <p className="py-4 text-center font-sans text-xs text-ash">{emptyMessage}</p>
          )}

          {entries.length > 0 && filtered && filtered.length === 0 && (
            <p className="py-4 text-center font-sans text-xs text-ash">Нічого не знайдено.</p>
          )}
