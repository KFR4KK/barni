...
import { TagPicker, type PickedTag } from "@/components/projects/TagPicker";

interface ProjectFormProps {
  mode: "create" | "edit";
  /** Required when mode === "edit"; ignored otherwise. */
  project?: Project;
  /** Edit mode only — this project's current tags (built-in + its own
   * custom ones), fetched server-side (app/projects/[slug]/edit/page.tsx
   * via lib/tags.ts's getProjectTags) so the picker starts pre-selected
   * instead of empty. */
...
export function ProjectForm({ mode, project, initialTags }: ProjectFormProps) {
  ...
  const [selectedTags, setSelectedTags] = useState<PickedTag[]>(
    () =>
      initialTags?.map((tag) => ({ id: tag.id, name: tag.name, isBuiltIn: tag.isBuiltIn })) ?? []
  );
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    // Phase 10 — Tags. Real tags (built-in, or an already-persisted
    // custom tag on the edit form) go in `tagIds`; anything just typed
    // via "Створити власний тег" this session (`isStaged`) has no real
    // Tag.id yet and goes in `customTagNames` instead — the server
    // creates and attaches those as part of this same request (see
    // app/api/projects/route.ts / app/api/projects/[slug]/route.ts).
    const tagIds = selectedTags.filter((tag) => !tag.isStaged).map((tag) => tag.id);
    const customTagNames = selectedTags.filter((tag) => tag.isStaged).map((tag) => tag.name);
    const payload =
      mode === "create"
        ? { title, description, coverImage, shortDescription, githubUrl, externalUrl, tagIds, customTagNames }
        : {
            title,
            description,
            coverImage,
