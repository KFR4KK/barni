...
async function resolveNotificationDetails(
  ...
  switch (type) {
    ...
    case "PROFILE_COMMENT": {
      ...
      const username = comment?.profileUser.username ?? null;
      return {
    ...
    case "POST_COMMENT": {
      // A Post has no page of its own (see lib/posts.ts's own comment) —
      // it only ever appears embedded in its author's profile
      // (PostsSection), so unlike PROJECT_COMMENT's link to the project
      // itself, this links to that profile page instead.
      if (!entityId) return { message: `${actorName} залишив коментар до вашого поста`, href: null };
      const comment = await prisma.postComment.findUnique({
        where: { id: entityId },
        select: { post: { select: { user: { select: { username: true } } } } },
      });
      const username = comment?.post.user.username ?? null;
      return {
        message: `${actorName} залишив коментар до вашого поста`,
        href: username ? buildProfileURL(username) : null,
      };
    }
    ...
    case "COMMENT_REPLY": {
      ...
      // replies to ProfileComment, ProjectComment, *and* PostComment
      // threads (per each phase's own brief — one NotificationType, not
      // three). The three tables have disjoint cuid id spaces, so trying
      // each in turn is the only way to tell which without a
      // discriminator column none of those briefs ask for.
      if (entityId) {
        ...
        if (profileReply) {
          const username = profileReply.profileUser.username ?? null;
          return {
        ...
        const postReply = await prisma.postComment.findUnique({
          where: { id: entityId },
          select: { post: { select: { user: { select: { username: true } } } } },
        });
        if (postReply) {
          const username = postReply.post.user.username ?? null;
          return {
            message: `${actorName} відповів на ваш коментар`,
            href: username ? buildProfileURL(username) : null,
          };
