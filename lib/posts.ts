...
export type PostWithAuthor = Post & { author: FollowListEntry };

const POST_LIST_INCLUDE = { user: { include: { profile: true } } } as const;

function toPostWithAuthor(row: Post & { user: Parameters<typeof toFollowListEntry>[0] }): PostWithAuthor {
  const { user, ...post } = row;
  return { ...post, author: toFollowListEntry(user) };
}
