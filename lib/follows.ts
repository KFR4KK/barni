...
export interface FollowListEntry {
  ...
  bio: string | null;
  /** The user's own username, for linking to `/members/[username]`, if
   * they've claimed a Profile — null otherwise (Profile no longer has
   * its own slug, see prisma/schema.prisma's comment on that model).
   * Also doubles as "does this user have a claimed profile", which is
   * what decides whether a Discord badge renders for them (see
   * components/members/FollowListItem.tsx). */
  profileUsername: string | null;
  /** Only meaningful when `profileUsername` isn't null — Phase 4's
   * Discord membership sync only ever writes to a claimed Profile. */
