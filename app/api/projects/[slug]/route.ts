...
function readUpdateInput(body: unknown): UpdateBody | null {
  ...
  // Phase 10 — Tags. `tagIds` is the picker's complete current selection
  // of already-real tags (built-in + this project's existing custom
  // tags) — a full replace, not a diff (see replaceProjectTags' own
  // comment). `customTagNames` is anything newly typed via "Створити
  // власний тег" during this edit session, not yet a real Tag row; both
  // are applied after the core project fields save below.
  ...
  const cleanCustomTagNames = Array.isArray(customTagNames)
    ...
    : [];
  if (cleanTagIds.length + cleanCustomTagNames.length > MAX_TAGS_PER_PROJECT) return null;
...
export async function PATCH(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  // Phase 10 — Tags. Runs after the core project fields save
  // successfully, same ordering as the cover-image cleanup below.
  await replaceProjectTags(existing.id, session.user.id, input.tagIds);
  for (const name of input.customTagNames) {
    try {
      await createCustomTag(existing.id, session.user.id, name);
    } catch (error) {
      console.error("[api/projects/:slug] failed to create custom tag on update:", error);
    }
  }
